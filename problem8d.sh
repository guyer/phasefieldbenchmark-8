#!/bin/bash
# invoke with
# $ sbatch --ntasks=16 problem8d.sh <env> <solver> <tag> [args]
...
OMP_NUM_THREADS=1 FIPY_SOLVERS=$SOLVER mpiexec -n 1 smt run --tag $TAG -n $SLURM_NTASKS --main benchmark8d.py params8d.yaml "$@"
